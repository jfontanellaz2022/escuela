...
class AlumnoRindeMateria {
    ...
    *****************************************************************************************************************/
	...
	public function actualizarNotasActasExamenes($calendario_id) {
		$this->getConection();
		$sql = "DELETE FROM alumno_rinde_materia 
				WHERE idCalendario = ? and llamado=2 and concat(idAlumno,idMateria,idCalendario) in 
				       ( SELECT concat(idAlumno,idMateria,idCalendario) FROM alumno_rinde_materia 
		                 WHERE idCalendario = ? and estado_final='Aprobo')";
		$stmt = $this->conection->prepare($sql);
		return $stmt->execute([$calendario_id,$calendario_id]);				 
	}
